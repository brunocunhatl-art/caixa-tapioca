...
function App(){
 ...
 const menuItems=[['novo','Novo pedido','➕'],['pedidos','Pedidos','🧾'],['cozinha','Tela cozinha','🔥'],['financeiro','Financeiro','📊'],['cardapio','Cardápio','🍔'],['cupons','Cupons','🏷️'],['recibo','Configurações','⚙️']];
 return <div className="app adminApp">
  <audio ref={beepRef} src="/pedido.wav" preload="auto"></audio>
  <aside className="sidebar">
    <div className="brand"><img src="/logo-verbohub.jpeg"/><div><b>Verbo Hub</b><span>Administração</span></div></div>
    <div className={open?'statusPill open':'statusPill closed'}>{open?'🟢 Loja aberta':'🔴 Loja fechada'}</div>
    <nav className="sideNav">{menuItems.map(t=><button key={t[0]} className={tab===t[0]?'on':''} onClick={()=>setTab(t[0])}><span>{t[2]}</span>{t[1]}</button>)}</nav>
    <div className="sideFooter"><small>{today()}</small><small>{syncStatus}</small></div>
  </aside>
  <section className="mainArea">
  <header className="top adminTop"><div><p className="eyebrow">Painel operacional</p><h1>{menuItems.find(x=>x[0]===tab)?.[1] || 'Verbo Hub'}</h1><p>{open?'🟢 Loja aberta':'🔴 Loja fechada'} • {today()} • {syncStatus}</p></div><div className="storeControls"><label>Tempo <input type="number" value={estimatedMinutes} onChange={e=>setEstimatedMinutes(e.target.value)}/> min</label><input placeholder="Mensagem da loja" value={storeMessage} onChange={e=>setStoreMessage(e.target.value)}/><button className="ghost" onClick={updateStoreConfig}>Salvar</button><button className={open?'danger':'primary'} onClick={()=>setOpenStore(!open)}>{open?'Fechar loja':'Abrir loja'}</button></div></header>
