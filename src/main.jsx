...
const CLOUD_TABLE = 'verbohub_state';
const store = {
...
async function cloudSet(key, value){
  if(!supabase) return;
  const { error } = await supabase.from(CLOUD_TABLE).upsert({ key, value, updated_at: new Date().toISOString() });
  if(error) console.warn('Supabase salvar:', error.message);
}
async function cloudGet(key){
  if(!supabase) return undefined;
  const { data, error } = await supabase.from(CLOUD_TABLE).select('value').eq('key', key).maybeSingle();
  if(error) { console.warn('Supabase carregar:', error.message); return undefined; }
  return data?.value;
}
async function cloudSeedIfEmpty(key, value){
  if(!supabase) return;
  const current = await cloudGet(key);
  if(current === undefined || current === null) await cloudSet(key, value);
}
...
function calcItem(item){
  ...
  if(isCuscuzBase) return base + adds.slice(4).reduce((s,a)=>s+Number(a.price),0);
  if(isCuscuzPremium) return base + adds.reduce((s,a)=>s+Number(a.price),0);
  return base + adds.reduce((s,a)=>s+Number(a.price),0);
}
...
function App(){
 ...
 useEffect(()=>{
   ...
   async function loadCloud(){
     if(!supabase) return;
     await cloudSeedIfEmpty('vh_products_v3', seedProducts);
     await cloudSeedIfEmpty('vh_adds_v3', seedAdds);
     await cloudSeedIfEmpty('vh_orders_v3', []);
     const [cp,ca,co,copen,ccashOpen,ccashClose] = await Promise.all([
       cloudGet('vh_products_v3'), cloudGet('vh_adds_v3'), cloudGet('vh_orders_v3'),
       cloudGet('vh_store_open'), cloudGet('vh_cash_open'), cloudGet('vh_cash_close')
     ]);
     if(!mounted) return;
     if(cp) { setProducts(cp); store.set('vh_products_v3', cp); }
     if(ca) { setAdds(ca); store.set('vh_adds_v3', ca); }
     if(co) { setOrders(co); store.set('vh_orders_v3', co); }
     if(typeof copen === 'boolean') { setOpen(copen); store.set('vh_store_open', copen); }
     if(ccashOpen !== undefined && ccashOpen !== null) { setCashOpen(Number(ccashOpen)||0); store.set('vh_cash_open', Number(ccashOpen)||0); }
     if(ccashClose !== undefined && ccashClose !== null) { setCashClose(Number(ccashClose)||0); store.set('vh_cash_close', Number(ccashClose)||0); }
   }
   loadCloud();
   const channel = supabase?.channel('verbohub-state-sync')
     .on('postgres_changes', { event: '*', schema: 'public', table: CLOUD_TABLE }, payload => {
       const row = payload.new;
       if(!row) return;
       store.set(row.key, row.value);
       if(row.key==='vh_products_v3') setProducts(row.value || seedProducts);
       if(row.key==='vh_adds_v3') setAdds(row.value || seedAdds);
       if(row.key==='vh_orders_v3') setOrders(row.value || []);
       if(row.key==='vh_store_open') setOpen(Boolean(row.value));
       if(row.key==='vh_cash_open') setCashOpen(Number(row.value)||0);
       if(row.key==='vh_cash_close') setCashClose(Number(row.value)||0);
     }).subscribe();
   return ()=>{ mounted=false; if(channel) supabase.removeChannel(channel); };
 }, []);
 const saveOrders=v=>{setOrders(v);store.set('vh_orders_v3',v);cloudSet('vh_orders_v3',v)};
 const saveProducts=v=>{setProducts(v);store.set('vh_products_v3',v);cloudSet('vh_products_v3',v)};
 const saveAdds=v=>{setAdds(v);store.set('vh_adds_v3',v);cloudSet('vh_adds_v3',v)};
 const setOpenStore=v=>{
   if(v){ const val=prompt('Com quanto em dinheiro está abrindo a loja?','0'); if(val===null) return; const n=Number(String(val).replace(',','.'))||0; setCashOpen(n); store.set('vh_cash_open',n); cloudSet('vh_cash_open',n); setCashClose(0); store.set('vh_cash_close',0); cloudSet('vh_cash_close',0); }
   else { const val=prompt('Quanto tem em dinheiro no caixa ao fechar?','0'); if(val===null) return; const n=Number(String(val).replace(',','.'))||0; setCashClose(n); store.set('vh_cash_close',n); cloudSet('vh_cash_close',n); }
   setOpen(v);store.set('vh_store_open',v);cloudSet('vh_store_open',v)
 };
 ...
 const createOrder=()=>{ if(!cart.length) return alert('Adicione produtos ao pedido.'); const o={id:uid(),num:orders.length+1,date:new Date().toISOString(),customer:customer||'Cliente balcão',items:cart,obs,status:'aberto',discount:0,extra:0,payments:[],fiado:false}; saveOrders([o,...orders]); setCart([]); setCustomer(''); setObs(''); setTab('pedidos'); };
 ...
 return <div className="app">
  <header className="top"><img src="/logo-verbohub.jpeg"/><div><h1>Verbo Hub</h1><p>{open?'🟢 Loja aberta':'🔴 Loja fechada'} • {today()}</p></div><button className={open?'danger':'primary'} onClick={()=>setOpenStore(!open)}>{open?'Fechar loja':'Abrir loja'}</button></header>
